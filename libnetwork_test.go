...
func TestNetworkConfig(t *testing.T) {
	...
	// Verify config network is not supported for global scope network for now
	configNetwork, err = controller.NewNetwork("overlay", "config_network0", "",
		libnetwork.NetworkOptionConfigOnly(),
	...
	// Verify a network cannot be created with configFrom and other user configurations
	for i, opt := range []libnetwork.NetworkOption{
		...
		_, err = controller.NewNetwork("", "testBR", "",
	...
	network, err := controller.NewNetwork("", "testBR", "",
