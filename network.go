...
func (n *network) validateConfiguration() error {
	if n.configFrom != "" {
		...
		if n.ipamType != "" && n.ipamType != defaultIpamForNetworkType(n.networkType) ||
			n.enableIPv6 || n.internal || n.attachable ||
			len(n.labels) > 0 || len(n.ipamOptions) > 0 ||
			len(n.ipamV4Config) > 0 || len(n.ipamV6Config) > 0 {
			return types.BadRequestErrorf("user specified configurations are not supported if the network depends on a configuration network")
		}
		if len(n.generic) > 0 {
...
func (n *network) applyConfigurationTo(to *network) error {
	...
	to.internal = n.internal
	to.attachable = n.attachable
	if len(n.labels) > 0 {
	...
	if n.ConfigFrom() != "" {
		if t, err := c.getConfigNetwork(n.ConfigFrom()); err == nil {
			if err := t.getEpCnt().DecEndpointCnt(); err != nil {
				logrus.Warnf("Failed to update reference count for network configOnly %q on removal of network %q: %v",
					t.Name(), n.Name(), err)
			}
		} else {
